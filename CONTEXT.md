...
### 動的レイアウト `compute_layout(show_title: bool, num_games: int)`
- `show_title=True, num_games=8` : ヘッダー **96px** + 2列×4行 = **8スロット**
- `show_title=True, num_games=10`: ヘッダー **96px** + 2列×5行 = **10スロット**（カード縦方向に縮小）
- `show_title=False, num_games=8` : ヘッダーなし + 2列×4行 = **8スロット**
- `show_title=False, num_games=10`: ヘッダーなし + 2列×5行 = **10スロット**
- どちらも出力は **1920×1080 px 固定**。カード間マージン: **4 px**（定数 `MARGIN`）
- 下端 **`FOOTER_H=36px`** をフッター帯として確保（`grid_h = CANVAS_H - header_h - FOOTER_H`）
- フッター帯: ヘッダーと対称デザイン（`theme["header"]` 背景 + `theme["accent"]` 4px ライン）
- ウォーターマーク "Generated by SteamPosterMaker" はフッター帯内に縦中央揃えで配置
- `title_max_h` を縮小して `review_max_h` を最大化（レビュー文の表示エリアを広く）
- ゲーム数セレクターは設定 popover 内のラジオボタン（`num_games_sel` セッション key）で制御
...
- ~~プレイ人数~~: v12 にて廃止。`review_y` がタイトル直下に詰まりレビュー表示スペースが拡大

### スロット並べ替えモード
- ヘッダー行の **「並び替え」ボタン** で `reorder_mode` フラグをトグル
- `True` 時: `sort_items` 縦リスト（ドラッグ用）を上段に表示 → 下段に**カードグリッドも常時表示**
    - `render_slot_card(i, disabled=True)` で編集ボタンを無効化（ホバーエフェクトなし）
    - ゲーム情報を視認しながら並べ替えできる
- `False` 時: カードグリッドのみ表示（`disabled=False`、通常通り編集可能）
...
- **ダイアログ**:
    ...
    - ダイアログ内ウィジェット: `dlg_review_{i}`, `dlg_q_{i}` キーで管理
...
- ~~[x] スロットカード: プレイ人数を「プレイ人数: ○○」形式で表示~~ → v12 で廃止
...
- [x] ポスターフッター帯（`FOOTER_H=36px`）導入・ウォーターマークとカードの重なりを解消
- [x] `HEADER_H: 120→96px` に縮小（フッターとのバランス調整）
- [x] `render_slot_card(disabled=)` 追加: 並び替えモード時に編集ボタンを無効化
...
- `_DEV_SAMPLE_GAMES` は API を呼ばずに直接 session_state へ書き込む。本番のゲームデータと同じ dict 構造（`app_id`, `title`, `image_url`, `price`, `age_restricted`, `review`）を維持すること。`players` キーは v12 で削除済み
